...
#assistant functions
    ...
    def callback_listener(self,data):
	    ...
	    rospy.loginfo("I spoke:: %s",response)
            if(self.count==1):#The number of questions
	        response_publisher.publish(response)
            	rospy.Subscriber("voi_ori", Float32, self.voi_ori_callback)
            	rospy.Subscriber("conf_ori", Float32, self.conf_ori_callback)
	    response_publisher.publish(response)
    ...
    def move(self,rot_origin,pos_origin,movecb):
        '''
        ...
        self.move_base_client.send_goal(goal,self.count_people)
        '''
        
	self.ori_msg.angular.z=1
	self.ori_pub.publish(self.ori_msg)
	rospy.sleep(1)
	self.ori_pub.publish(self.ori_msg)
	rospy.sleep(1)
    ...
    def count_people(self):#count number of people
